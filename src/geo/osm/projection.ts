...
export function geoToBlock(point: GeoPoint, origin: GeoPoint, blockMetres: number = BLOCK_METRES): BlockPoint {
  const xMetres = (point.lon - origin.lon) * 111_320 * Math.cos(origin.lat * Math.PI / 180);
  const zMetres = -(point.lat - origin.lat) * 110_540;
  const x = Math.round(xMetres / blockMetres);
  const z = Math.round(zMetres / blockMetres);
  return {
    x: Object.is(x, -0) ? 0 : x,
    z: Object.is(z, -0) ? 0 : z,
  };
